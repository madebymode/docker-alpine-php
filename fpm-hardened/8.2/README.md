...
| Stage | Base | Purpose |
|-------|------|---------|
| `builder` | `dhi.io/php:8.2-alpine3.22-dev` | Compiles extensions against DHI's exact PHP binary using `$PHP_SRC_DIR` |
| `go-builder` | `golang:1.26-alpine3.22` | Builds the static `fcgi-health` probe binary |
| runtime | `dhi.io/php:8.2-alpine3.22-fpm` | CIS-hardened, non-root, read-only rootfs |
...
- `$PHP_PREFIX=/opt/php-8.2` — DHI's prefix; all PHP paths live here
...
services:
  php:
    ...
    environment:
      - PHP_INI_SCAN_DIR=/opt/php-8.2/etc/php/conf.d:/opt/mode/conf.d
```
