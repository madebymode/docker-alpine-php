...
| Stage | Base | Purpose |
|-------|------|---------|
| `builder` | `dhi.io/php:8.3-alpine3.24-dev` | Compiles extensions against DHI's exact PHP binary using `$PHP_SRC_DIR` |
| `go-builder` | `golang:1.26-alpine3.22` | Builds the static `fcgi-health` probe binary |
| runtime | `dhi.io/php:8.3-alpine3.24-fpm` | CIS-hardened, non-root, read-only rootfs |
...
- PHP is configured with prefix `/usr` and config under `/etc/php-8.3`
...
```dockerfile
FROM dhi.io/php:8.3-alpine3.24-dev AS builder
...
FROM dhi.io/php:8.3-alpine3.24-fpm
COPY --from=builder /opt/dhi-extensions /usr/lib/php/extensions
COPY custom-ext.ini /etc/php-8.3/conf.d/
```
...
services:
  php:
    ...
    environment:
      - PHP_INI_SCAN_DIR=/etc/php-8.3/conf.d:/opt/mode/conf.d
```
